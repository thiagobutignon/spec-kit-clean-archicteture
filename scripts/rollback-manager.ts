...
/**
 * Rollback Manager for Failed Steps
 * Provides safe rollback capabilities when steps fail
 */

interface RollbackSnapshot {
  stepId: string;
  timestamp: Date;
  files: Map<string, string>; // filepath -> content before change
  gitStatus: string;
  branch: string;
}
...
class RollbackManager {
   ...
   */
  async rollbackMultiple(yamlPath: string): Promise<void> {
   ...
   * Clean old snapshots
   */
  async cleanSnapshots(daysOld: number = 7): Promise<void> {
    const snapshots = await fs.readdir(this.snapshotsDir);
   ...
   */
  async createFullBackup(projectPath: string = '.'): Promise<string> {
