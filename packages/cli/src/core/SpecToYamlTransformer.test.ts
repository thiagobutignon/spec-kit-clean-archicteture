...
describe('SpecToYamlTransformer', () => {
  let transformer: SpecToYamlTransformer;
  ...
  describe('transformTaskObject', () => {
    ...
    it('should handle different layers correctly', async () => {
      const layers: Array<Task['layer']> = ['domain', 'data', 'infra', 'presentation', 'main'];
    ...
    it('should include commit message with proper format', async () => {
      ...
      const workflow = await transformer.transformTaskObject(task);
      const commitStep = workflow.domain_steps?.find(s =>
        s.type === 'validation' && s.validation_script?.includes('git commit')
      );
    ...
    it('should include validation step with proper commands', async () => {
      ...
      const workflow = await transformer.transformTaskObject(task);
      const validationStep = workflow.domain_steps?.find(s =>
        s.type === 'validation' && s.validation_script?.includes('npm test')
      );
