...
import { YamlWorkflow, YamlStep, Task, GitFlowConfig, YamlMetadata } from '../types/YamlWorkflow.js';
...
export class SpecToYamlTransformer {
   ...
   */
  async transformTaskObject(task: Task): Promise<YamlWorkflow> {
    return this.createYamlWorkflow(task);
  }
   ...
   */
  private async parseTaskFromMarkdown(taskId: string, taskPath: string): Promise<Task> {
    ...
    const layer = this.extractField(taskContent, 'Layer', '(domain|data|infra|presentation|main)') as Task['layer'];
    const storyPoints = parseInt(this.extractField(taskContent, 'Story Points', '(\\d+)') || '1');
    const priority = this.extractField(taskContent, 'Priority', '(Primary|Secondary|Integration)') as Task['priority'];
   ...
   */
  private extractField(content: string, fieldName: string, pattern: string): string {
   ...
   */
  private createYamlWorkflow(task: Task): YamlWorkflow {
   ...
   */
  private createStepsForTask(task: Task): YamlStep[] {
...
echo "Created/switched to branch: ${branchName}"
      `.trim()
    };
  }
   ...
   */
  private createDirectoryStep(task: Task): YamlStep {
   ...
   */
  private createImplementationSteps(task: Task): YamlStep[] {
    const steps: YamlStep[] = [];
   ...
   */
  private createTestSteps(task: Task): YamlStep[] {
    const steps: YamlStep[] = [];
   ...
   */
  private createValidationStep(task: Task): YamlStep {
    return {
      id: `validate-${task.id}`,
      type: 'validation',
      status: 'PENDING',
...
Generated by SpecToYamlTransformer from spec-kit analysis."
   ...
   */
  async saveWorkflowAsYaml(workflow: YamlWorkflow, outputPath: string): Promise<void> {
  ...
  private getFeatureFromTask(task: Task): string {
    // Extract feature name from task ID or title
    // For now, assume project-init as the feature
    return 'project-init';
  }

  private getDirectoriesForLayer(layer: string): string[] {
  ...
  private getFilesForTask(task: Task): Array<{path: string, template: string}> {
  ...
  private getTestFilesForTask(task: Task): Array<{path: string, template: string}> {
    const files = this.getFilesForTask(task);
  ...
  private getProjectEntityTemplate(): string {
    return `/**
