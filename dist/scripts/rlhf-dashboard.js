...
class RLHFDashboard {
    ...
    async displayMetricsSummary() {
        ...
        const avgDuration = metrics.reduce((acc, m) => acc + m.duration, 0) / totalRuns;
        // Calculate average RLHF score
        const avgRLHFScore = metrics.reduce((acc, m) => acc + (m.rlhfScore || 0), 0) / totalRuns;
    ...
    async displayRLHFScoreDistribution() {
        const metricsFile = path.join(this.dataDir, 'metrics.json');
        if (!await fs.pathExists(metricsFile)) {
            return;
        }
        ...
        metrics.forEach((m) => {
            ...
            if (score <= -2)
                scoreDistribution['catastrophic (-2)']++;
            else if (score <= -1)
                scoreDistribution['runtime error (-1)']++;
            else if (score <= 0)
                scoreDistribution['low confidence (0)']++;
            else if (score <= 1)
                scoreDistribution['good (+1)']++;
            else
                scoreDistribution['perfect (+2)']++;
        });
    ...
    async displayImprovementStatus() {
    ...
    createBar(percentage, width, color = 'blue') {
