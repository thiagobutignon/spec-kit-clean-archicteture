...
class EnhancedRLHFSystem {
    ...
    progressCallback;
    progressFile;
    constructor(contextPath, cacheExpiry) {
     ...
     */
    async analyzeExecution(yamlPath, layerInfo) {
        ...
        for (const step of steps) {
            ...
            processedSteps++;
            this.reportProgress(`Processing step ${processedSteps}/${totalSteps}`, 10 + (processedSteps / totalSteps * 60));
        }
     ...
     */
    async calculateLayerScore(stepType, success, layerInfo, errorMessage, stepData) {
     ...
     */
    applyLayerPatterns(score, template, layerInfo) {
     ...
     * Apply template-defined score impacts
     */
    applyTemplateScoreImpacts(score, stepData, layerInfo) {
     ...
     */
    async updatePatterns(metrics, layerInfo) {
        ...
        await fs.writeJson(this.patternsFile, Object.fromEntries(patterns), { spaces: 2 });
    }
    /**
     * Generate layer-aware fix suggestions
     */
    generateLayerAwareFix(errorType, layerInfo) {
     ...
     */
    async generateImprovements(metrics, layerInfo) {
     ...
     */
    inferTemplatePath(patternKey, layerInfo) {
     ...
     */
    async generateLayerReport(layerInfo) {
        ...
        const report = {
            ...
            summary: {
                ...
                avgDuration: filteredMetrics.reduce((acc, m) => acc + m.duration, 0) / filteredMetrics.length
            },
        ...
        await fs.writeJson(path.join(this.dataDir, reportFile), report, { spaces: 2 });
        this.logger.log(`✅ Layer-aware report saved to ${reportFile}`);
    }
    /**
     * Original calculateScore method (kept for compatibility)
     */
    async calculateScore(stepType, success, errorMessage, stepData) {
     ...
     */
    analyzeFailureSeverity(errorMessage, stepType, stepData) {
     ...
     */
    analyzeSuccessQuality(stepType, stepData) {
        let score = 1;
    ...
    saveProgress(message, percentage) {
        try {
    ...
    switch (command) {
        ...
        case 'score':
            if (args[0] && args[1]) {
                ...
                const score = await rlhf.calculateLayerScore(args[0], args[1] === 'true', layerInfo, args[2]);
...
export { EnhancedRLHFSystem, EnhancedRLHFSystem as RLHFSystem };
