...
const mockExecSyncWithString = (output) => output;
...
`;
    ...
    describe('installSerena', () => {
        it('should use correct command for Serena installation', async () => {
            ...
            expect(childProcess.execSync).toHaveBeenCalledWith(expect.stringContaining('uvx --from git+https://github.com/oraios/serena'), expect.any(Object));
            expect(childProcess.execSync).toHaveBeenCalledWith(expect.stringContaining('serena start-mcp-server --context ide-assistant --project'), expect.any(Object));
        });
        it('should quote project path with spaces', async () => {
            ...
            expect(childProcess.execSync).toHaveBeenCalledWith(expect.stringContaining('"/path with spaces/project"'), expect.any(Object));
        });
    ...
    describe('installAll', () => {
        it('should handle already exists errors as skipped', async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('⏭️  Serena - Already installed (skipped)'));
            consoleSpy.mockRestore();
        });
        it('should handle multiple already exists errors correctly', async () => {
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
            const serenaError = new Error('Installation failed');
            serenaError.stderr = Buffer.from('MCP server serena already exists in local config');
            const chromeError = new Error('Installation failed');
        ...
        it('should differentiate between genuine failures and already exists', async () => {
            const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => { });
            const alreadyExistsError = new Error('Installation failed');
            alreadyExistsError.stderr = Buffer.from('MCP server serena already exists in local config');
            const realError = new Error('Installation failed');
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('⏭️  Serena - Already installed (skipped)'));
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('❌ Chrome DevTools installation failed'));
            consoleSpy.mockRestore();
        });
    });
});
