...
export const SUPPORTED_MCP_SERVERS = ['serena', 'context7', 'chrome-devtools', 'playwright'];
...
export class MCPInstaller {
     ...
     */
    handleInstallError(error, serverName, displayName, report) {
    ...
    async installAll(config) {
        ...
        if (config.installSerena) {
            try {
                ...
                this.handleInstallError(error, 'serena', 'Serena', report);
            }
        }
        else {
        ...
        if (config.installContext7 && config.context7ApiKey) {
            try {
                ...
                this.handleInstallError(error, 'context7', 'Context7', report);
            }
        }
        ...
        if (config.installChromeDevTools) {
            try {
                ...
                this.handleInstallError(error, 'chrome-devtools', 'Chrome DevTools', report);
            }
        }
        else {
        ...
        if (config.installPlaywright) {
            try {
                ...
                this.handleInstallError(error, 'playwright', 'Playwright', report);
            }
        }
        else {
     ...
     */
    async verifyInstallation(retries = 2) {
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                for (const match of matches) {
                    ...
                    if (SUPPORTED_MCP_SERVERS.includes(serverName)) {
 ...
 */
export async function promptMCPInstallation() {
    console.log(chalk.cyan.bold('\n🔧 MCP Server Setup\n'));
