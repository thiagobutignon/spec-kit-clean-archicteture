...
import { mapStepTypeToCommitType, generateCommitMessage, shouldCommitStep, formatCommitMessageForShell, createQualityCheckResult, DEFAULT_COMMIT_CONFIG, } from './commit-generator';
...
describe('generateCommitMessage', () => {
    it('should generate conventional commit for create_file with domain scope', () => {
        const message = generateCommitMessage('create_file', 'Create Product entity', 'src/domain/models/product.ts');
        // Description already includes entity name and type, so it won't be enhanced
        expect(message).toBe('regent(domain): create Product entity\n\n🤖 Generated with Claude Code\n\nCo-Authored-By: Claude <noreply@anthropic.com>');
    });
    it('should generate conventional commit for refactor_file with data scope', () => {
        const message = generateCommitMessage('refactor_file', 'Refactor logic', 'src/data/usecases/create-user.ts');
    ...
    it('should generate conventional commit for delete_file with infra scope', () => {
        const message = generateCommitMessage('delete_file', 'Remove deprecated adapter', 'src/infra/cache/old-cache.ts');
    ...
    it('should lowercase first letter of description', () => {
        const message = generateCommitMessage('create_file', 'Add new user validator', 'src/domain/validators/user-validator.ts');
        expect(message).toContain('regent(domain): add new user validator');
    });
    it('should use core scope when no path provided', () => {
        const message = generateCommitMessage('create_file', 'Add configuration file');
    ...
    it('should return null when commits are disabled in config', () => {
        ...
        const message = generateCommitMessage('create_file', 'Create file', 'src/domain/models/user.ts', disabledConfig);
    ...
    it('should use custom co-author from config', () => {
        ...
        const message = generateCommitMessage('create_file', 'Create file', 'src/domain/models/user.ts', customConfig);
        expect(message).toContain('Co-Authored-By: Custom Bot <bot@example.com>');
    });
    it('should extract scope from complex path', () => {
        const message = generateCommitMessage('create_file', 'Create order entity', 'product-catalog/src/features/order-management/shared/domain/entities/order.ts');
        expect(message).toContain('regent(domain):');
    });
    it('should handle presentation scope', () => {
        const message = generateCommitMessage('create_file', 'Create user controller', 'src/presentation/controllers/user-controller.ts');
        expect(message).toContain('regent(presentation):');
    });
    it('should handle main scope', () => {
        const message = generateCommitMessage('create_file', 'Create user factory', 'src/main/factories/user-factory.ts');
