...
import { resolveLogDirectory, resolveDebugDirectory, resolveRLHFDirectory } from './log-path-resolver';
describe('Log Path Resolver', () => {
    describe('resolveLogDirectory', () => {
        ...
        it('should throw error for null context path', () => {
            expect(() => resolveLogDirectory(null)).toThrow('Context path is required for resolveLogDirectory');
        });
        it('should throw error for non-string context path', () => {
            expect(() => resolveLogDirectory(123)).toThrow('Context path must be a string, received: number');
        });
    ...
    describe('resolveDebugDirectory', () => {
        ...
        it('should throw error for null context path', () => {
            expect(() => resolveDebugDirectory(null)).toThrow('Context path is required for resolveDebugDirectory');
        });
        it('should throw error for non-string context path', () => {
            expect(() => resolveDebugDirectory({})).toThrow('Context path must be a string, received: object');
        });
    });
    describe('resolveRLHFDirectory', () => {
        ...
        it('should throw error for non-string context path', () => {
            expect(() => resolveRLHFDirectory([])).toThrow('Context path must be a string, received: object');
        });
    });
    describe('Edge Cases', () => {
