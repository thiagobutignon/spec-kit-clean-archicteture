...
export function setupExecutionEnvVars(vars: {
  ...
  if (vars.nonInteractive) process.env.REGENT_NON_INTERACTIVE = vars.nonInteractive;
  if (vars.autoConfirm) process.env.REGENT_AUTO_CONFIRM = vars.autoConfirm;
  if (vars.strict) process.env.REGENT_STRICT = vars.strict;
  if (vars.ci) process.env.CI = vars.ci;
  if (vars.claudeCode) process.env.CLAUDE_CODE = vars.claudeCode;
  if (vars.aiOrchestrator) process.env.AI_ORCHESTRATOR = vars.aiOrchestrator;
}
