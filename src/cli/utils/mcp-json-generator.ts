 ...
 */
export const DEFAULT_MCP_CONFIG: MCPJsonConfig = {
  mcpServers: {
    serena: {
      command: 'uvx',
      args: [
 ...
 */
export async function mcpJsonExists(projectPath: string): Promise<boolean> {
  try {
 ...
 */
export async function readMcpJson(projectPath: string): Promise<MCPJsonConfig | null> {
  try {
    const mcpJsonPath = path.join(projectPath, '.mcp.json');
...
export async function generateMcpJson(
  projectPath: string,
  config: MCPJsonConfig = DEFAULT_MCP_CONFIG,
 ...
 */
export function displayMcpJsonInfo(): void {
 ...
 */
export function displayPrerequisites(): void {
